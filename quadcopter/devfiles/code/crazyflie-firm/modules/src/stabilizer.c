...
void stabilizerInit(void)
{
	...
	xTaskCreate(stabilizerTask, (char const * const )"STABILIZER", 2*configMINIMAL_STACK_SIZE, NULL, /*Piority*/2,
	            NULL);
...
//  pass &= motorsTest();//comment by titron
	pass &= imu6Test();
...
static void stabilizerTask(void* param)
{
	...
	while (1)
	{
		vTaskDelayUntil((portTickType * const ) &lastWakeTime, F2T(IMU_UPDATE_FREQ)); // 500Hz
		...
		if (imu6IsCalibrated())
		{
			...
			if (++attitudeCounter >= ATTITUDE_UPDATE_RATE_DIVIDER)
			{
				...
				controllerCorrectAttitudePID(eulerRollActual, eulerPitchActual, eulerYawActual, eulerRollDesired,
				                             eulerPitchDesired, -eulerYawDesired, &rollRateDesired, &pitchRateDesired,
				                             &yawRateDesired);
				attitudeCounter = 0;
			}
...
static void stabilizerAltHoldUpdate(void)
{
	...
	if (altHold)
	{
		...
		altHoldPIDVal = (pidAlpha) * altHoldPIDVal
		    + (1.f - pidAlpha)
		        * ((vSpeedAcc * vSpeedAccFac) + (vSpeedASL * vSpeedASLFac) + pidUpdate(&altHoldPID, asl, false));
