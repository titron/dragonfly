...
bool imu6Test(void)
{
	...
	if ((isHmc5883lPresent && !isMs5611Present) || (!isHmc5883lPresent && isMs5611Present))
	{
#ifdef __PRINT_INFO//comment by titron
 ...
 */
//static void __attribute__((used)) imuCalculateBiasMean(BiasObj* bias, Axis3i32* meanOut)//comment by titron
static void imuCalculateBiasMean(BiasObj* bias, Axis3i32* meanOut)
{
	uint32_t i;
	int32_t sum[GYRO_NBR_OF_AXES] = { 0 };
...
static bool imuFindBiasValue(BiasObj* bias)
{
	...
	if (bias->isBufferFilled)
	{
		...
		if (variance.x < GYRO_VARIANCE_THRESHOLD_X && variance.y < GYRO_VARIANCE_THRESHOLD_Y
