...
#if 1 //added by titron
	DBGMCU_Config(MOTORS_GPIO_TIM_M1_2_DBG, DISABLE);
	DBGMCU_Config(MOTORS_GPIO_TIM_M3_4_DBG, DISABLE);
#else
...
//		motorsSetRatio(MOTOR_M4, sequence[(step + 1) % 4]);
		motorsSetRatio(MOTOR_M3, sequence[(step + 1) % 4]);
//		motorsSetRatio(MOTOR_M2, sequence[(step + 1) % 4]);
//		motorsSetRatio(MOTOR_M1, sequence[(step + 1) % 4]);
