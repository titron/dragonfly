...
/* Synchronisation */
xSemaphoreHandle eskylink_dataRdy;
/* Data queue */
xQueueHandle eskylink_rxQueue;
...
static void interruptCallback()
{
  ...
  //To unlock RadioTask
  xSemaphoreGiveFromISR(eskylink_dataRdy, &xHigherPriorityTaskWoken);
...
static int receivePacket(CRTPPacket * pk)
{
  ...
  xQueueReceive( eskylink_rxQueue, pk, portMAX_DELAY);
...
static void eskylinkDecode(char* packet)
{
  ...
  xQueueSend(eskylink_rxQueue, &crtpPacket, 0);
}
...
static void eskylinkTask(void * arg)
{
  ...
  while (!state.paired)
  {
    xSemaphoreTake(eskylink_dataRdy, portMAX_DELAY);
  ...
  while(1)
  {
    if (xSemaphoreTake(eskylink_dataRdy, M2T(10))==pdTRUE)
    {
      ledseqRun(LED_GREEN, seq_linkup);
...
void eskylinkInit()
{
  ...
  vSemaphoreCreateBinary(eskylink_dataRdy);

  /* Queue init */
  eskylink_rxQueue = xQueueCreate(3, sizeof(CRTPPacket));

  eskylinkInitPairing();

    /* Launch the Radio link task */
  xTaskCreate(eskylinkTask, (char const * const)"EskyLink",
